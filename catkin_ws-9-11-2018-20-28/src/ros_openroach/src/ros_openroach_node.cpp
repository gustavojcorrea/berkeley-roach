...
void steering_dumb(ar_track_alvar_msgs::AlvarMarkers msg){
  ...
  float x_pos_tag = msg.markers[0].pose.pose.position.x;
  float y_pos_tag = msg.markers[0].pose.pose.position.y;
  float z_pos_tag = msg.markers[0].pose.pose.position.z;

  float line_pos = x_pos_tag;
  float TURN_SENS_INNER = 2;
 // float SPEED_PWM = 0.7;
...
int main(int argc, char** argv)  
{
  ...
  ros::Subscriber tag_pose_sub = edumip_node.subscribe("ar_pose_marker", 1, steering_dumb);
...
void balance_controller()
{
  ...
  pitch = imu_data.dmp_TaitBryan[0];
  roll  = imu_data.dmp_TaitBryan[1];
  yaw   = imu_data.dmp_TaitBryan[2];
...
*/	
  ...
  ROS_INFO("pitch, roll, yaw,dutyL, dutyR= %1.4f %1.4f %1.4f %1.4f %1.4f", pitch, roll, yaw, DUTY_L_GLOBAL, DUTY_R_GLOBAL);
  edumip_msgs::EduMipState  edumip_state;
  edumip_state.dutyL = DUTY_L_GLOBAL;
  edumip_state.dutyR = DUTY_R_GLOBAL;
  edumip_state.theta = pitch;
  edumip_state.phi   = roll;
  edumip_state.gamma = yaw;
